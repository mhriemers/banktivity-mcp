 ...
 */
export function registerTransactionTools(server: McpServer, db: BanktivityDatabase): void {
  server.registerTool(
    "get_transactions",
    {
    ...
    async ({ account_id, account_name, start_date, end_date, limit }) => {
      const accountId = resolveAccountId(db, account_id, account_name) ?? undefined;
  ...
  server.registerTool(
    ...
    async ({ query, limit }) => {
      const transactions = db.transactions.search(query, limit);
      return jsonResponse(transactions);
    }
  );
  ...
  server.registerTool(
    ...
    async ({ title, date, note, transaction_type, line_items }) => {
      ...
      for (const item of line_items) {
        const accountId = resolveAccountIdOrError(db, item.account_id, item.account_name);
        if (isErrorResponse(accountId)) {
          return errorResponse(`Account not found for line item: ${item.account_name || "no account specified"}`);
        }
        lineItems.push({
          accountId,
          amount: item.amount,
      ...
      const result = db.transactions.create({
        title,
        date,
        note,
      ...
      const transaction = db.transactions.getById(result.transactionId);
  ...
  server.registerTool(
    ...
    async ({ transaction_id, title, date, note, cleared }) => {
      ...
      const success = db.transactions.update(transaction_id, updates);
      ...
      return successResponse("Transaction updated successfully", { transaction });
    }
  );
  ...
  server.registerTool(
    ...
    async ({ transaction_id }) => {
      ...
      db.transactions.delete(transaction_id);

      return successResponse("Transaction deleted successfully", { deletedTransaction: transaction });
    }
  );
  ...
  server.registerTool(
    ...
    async ({ transaction_ids, cleared }) => {
      const updated = db.transactions.reconcile(transaction_ids, cleared);
